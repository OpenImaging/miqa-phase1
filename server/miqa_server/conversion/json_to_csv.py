...
def jsonObjectToCsvContent(jsonObject):
    ...
    for scan in scans:
        ...
        if scanId != parsedScanId:
            print('ERROR: expected scan id {0}, but found {1} instead'.format(
                scanId, parsedScanId))
            print(scan)
            continue
        ...
        if scanType != parsedScanType:
            print('ERROR: expected scan type {0}, but found {1} instead'.format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('inputfile',
                        type=str,
                        help='Absolute system path to JSON file to be converted')
    ...
    if not os.path.isfile(inputFilePath):
        print('Please absolute provide path to input JSON file, {0} is not valid'.format(inputFilePath))
        sys.exit(1)
