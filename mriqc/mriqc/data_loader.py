...
class Data():
    ...
    def _extract_iqm(self, iqms):
        ...
        '''
        iqm_vals = []
        # logger.info('raw iqms')
        # logger.info(iqms)
        iqms = iqms.split(';')[:-1]
        # logger.info('trimmed iqms')
        # logger.info(iqms)
        for iqm in iqms:
            label, val = iqm.split(':')
            val = float(val)
            iqm_vals.append(val)
        # logger.info('parse iqm vals')
        # logger.info(iqm_vals)
        return iqm_vals
    ...
    def get_feature_and_labels(self):
        ...
        for i in range(row_count):
            ...
            scan_type = self.data["scan_type"][i]
            iqm = self.data["IQMs"][i]
            # logger.info('{0} -> {1}'.format(sub_id, iqm))
            if(type(iqm)!=type(0.0)):
                try:
                    iqms = self._extract_iqm(iqm)
                    sub_ids.append((sub_id,scan_type))
                    ...
                    if(self.data['decision'][i] == 1 or self.data['decision'][i] == 2):
                        ...
                        logger.info('decision case fall through: {0}'.format(self.data['decision'][i]))
                except:
                    logger.info("encountered exception processing subject {0}, decision {1}, IQMs {2}".format(
                        sub_id, self.data.decision[i], iqm))

        if len(features) != len(labels):
            logger.info('Well this is going to be a problem ({0} feature arrays but {1} labels)'.format(len(features), len(labels)))
